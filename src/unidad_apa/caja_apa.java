 ...
 * @author Rojeru San
 */
public class caja_apa extends javax.swing.JInternalFrame {
...
static Conexion cc = new Conexion();
 ...
 int filas;
    ...
    private void numeros() {
        ...
        String num = "";
        String c = "";
        String SQL = "SELECT MAX(idventa) FROM caja_servicios_apa";
        try {
  ...
  private void Get_Data(){
        ...
        caja_apa.txttotalcaja.setText("TOTAL EN CAJA: L " + Math.rint((total) * 100) / 100);
  }
  //obtiene las factura que se a hecho al credito
  void cuentas_cobrar(){
        thishide.setVisible(true);
        String sql="select idventa as 'Codigo',codigo_examen as 'Codig Examen',cod_servicio as 'Codigo Servicio', paciente as 'Paciente', fecha as 'Fecha',estado_pago as 'Estado de Pago',abonado as 'Saldo Adeudado', pendiente as 'Saldo Pendiennte', total as 'Total (L)' from caja_servicios_apa where estado_pago='Crédito'";
        try{
         pst=con.prepareStatement(sql);
          rs= pst.executeQuery();
         tableCaja.setModel(DbUtils.resultSetToTableModel(rs));
        ...
        caja_apa.txttotalcaja.setText("TOTAL SALDO PENDIENTE: L " + Math.rint((total) * 100) / 100);
}
...
private void condicionIsv( ){
    if (rbisv.isSelected()){
        ...
        if (isv.getText().isEmpty() || isv.getText().equals("0")){
            ...
            new elegir.elegir_caja_apa(a, true).setVisible(true);   
        }    
...
//        double disv= Double.parseDouble(isvr);
    ...
    //obtener las ventas del dia al contado
      private void Get_Data_today(){
        ...
        thishide.setVisible(false);
        String sql="select idventa as 'Codigo',codigo_examen as 'Codig Examen',cod_servicio as 'Codigo Servicio', paciente as 'Paciente', fecha as 'Fecha',estado_pago as 'Estado de Pago',total as 'Total (L)' from caja_servicios_apa where estado_pago='Contado' and fecha='"+fecH+"'";
        try{
         pst=con.prepareStatement(sql);
          rs= pst.executeQuery();
         tableCaja.setModel(DbUtils.resultSetToTableModel(rs));
    ...
    void sales_date(){
       ...
       try {
            ...
            ResultSet rs = s.executeQuery("select idventa as 'Codigo',codigo_examen as 'Codigo Examen',cod_servicio as 'Codigo Servicio', paciente as 'Paciente', fecha as 'Fecha',estado_pago as 'Estado de Pago',total as 'Total (L)' from caja_servicios_apa WHERE fecha ='" + fecH + "' and estado_pago='Contado'");
        ...
        caja_apa.txttotalcaja.setText("TOTAL: L " + Math.rint((total) * 100) / 100);
            }
        } catch (Exception e) {
            JOptionPane.showMessageDialog(null, e);
        }          
    ...
    void receivable_date(){
       ...
       try {
            ...
            ResultSet rs = s.executeQuery("SELECT * FROM caja_servicios_apa WHERE fecha ='" + fecH + "' and estado_pago='Crédito'");
...
// traer los detalles de la factura con el codigo del examen
       private void show_detalle(){
        if (("E").equals(lblknowdetail.getText())){
        String sql="select codigo as 'Codigo',p_s as 'Prodcuto/Servicio',precio as 'Precio',cantidad as 'Cantidad',importe as 'Importe' from detalle_test_emergencia_apa where idventa='" + codetest.getText() + "' ";
        try{
         pst=con.prepareStatement(sql);
         rs= pst.executeQuery();
        ...
        String sql="select codigo as 'Codigo',p_s as 'Prodcuto/Servicio',precio as 'Precio',cantidad as 'Cantidad',importe as 'Importe' from detalle_test_hospitalizacion_apa where idventa='" + codetest.getText() + "' ";
        try{
         pst=con.prepareStatement(sql);
         rs= pst.executeQuery();
         tablaCaja.setModel(DbUtils.resultSetToTableModel(rs));
    ...
    void subtotal(){
        ...
        caja_apa.lblsubtotal.setText("" + Math.rint((total) * 100) / 100);
    }
    ...
    private void c_search_tblKeyReleased(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_c_search_tblKeyReleased
        ...
        try {
            ...
            if (thishide.isVisible()){
                ResultSet rs = s.executeQuery("SELECT * FROM caja_servicios_apa WHERE paciente LIKE '%"+name+"%' and estado_pago='Crédito'");
                ...
                caja_apa.txttotalcaja.setText("TOTAL SALDO PENDIENTE: L " + Math.rint((total) * 100) / 100);
                    ...
                    ResultSet rs = s.executeQuery("select idventa as 'Codigo',codigo_examen as 'Codig Examen',cod_servicio as 'Codigo Servicio', paciente as 'Paciente', fecha as 'Fecha',estado_pago as 'Estado de Pago',total as 'Total (L)' from caja_servicios_apa WHERE paciente LIKE '%"+name+"%' and estado_pago='Contado'");
    ...
    private void btnVenderActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnVenderActionPerformed
        ...
        if("vacio".equals(condiciones(Estado_actual))){
            ...
            if (txtcredito.getText().equals("") || txtcredito.getText().equals("0")) {
               ...
               // insertar datos en caja_servicios_apa
            if (rbcredito.isSelected()){
            String sql= "insert into caja_servicios_apa(idventa,cod_servicio,codigo_examen,paciente,fecha,estado_pago,abonado,pendiente,total) values ('"+numFac.getText()+"','" +lblknowdetail.getText()+"','" +codetest.getText()+"','" +txtpaciente.getText()+"','" +txtFecha.getText()+"','" +condiciones(Estado_actual)+"','" + Double.parseDouble(txtcredito.getText())+"','" +credito(Num)+"','" +lblTotal.getText()+ "')";
            pst=con.prepareStatement(sql);
            pst.execute();}else{
            String sql= "insert into caja_servicios_apa(idventa,cod_servicio,codigo_examen,paciente,fecha,estado_pago,total) values ('"+numFac.getText()+"','" +lblknowdetail.getText()+"','" +codetest.getText()+"','" +txtpaciente.getText()+"','" +txtFecha.getText()+"','" +condiciones(Estado_actual)+"','" + lblTotal.getText() + "')";
                pst=con.prepareStatement(sql);
                pst.execute();
            }
...
//funcion de factura
    ...
    private void btneditActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btneditActionPerformed
        ...
        if (P==0){
            try{
            con=Conexion.ConnectDB();
            String sql= "update caja_servicios_apa set estado_pago='" + "Contado" + "' where idventa='" + numFac.getText()+ "'";
            pst=con.prepareStatement(sql);
            pst.execute();
            SuccessAlert sa = new SuccessAlert(new JFrame(), true);
            sa.titulo.setText("¡HECHO!");
