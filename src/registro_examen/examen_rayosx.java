 ...
 int filas;
  ...
  private void Get_Data(){
    ...
    for (int i = 0; i <tablaCaja.getRowCount(); i++) {
        ...
        String sql= "insert into detalle_test_rayosx("
                + "idventa,"
                + "codigo,"
                + "id_paciente,"
                + "p_s,"
    ...
    private void show_detalle(){
        String sql="select codigo as 'Codigo',p_s as 'Producto/Servicio',precio as 'Precio',cantidad as 'Cantidad',importe as 'Importe' from detalle_test_rayosx where idventa='" + numFac.getText() + "' ";
        try{
         pst=con.prepareStatement(sql);
         rs= pst.executeQuery();
...
void sum_ingreso(){
     ...
     void edit_ingreso(){
         ...
         String sql= "UPDATE test_rayosx set total ='"+total+"' where codigo='"+numFac.getText()+"'";
         try{
            con=Conexion.ConnectDB();
            pst=con.prepareStatement(sql);
            pst.execute();
